...
<title>SEARCH | Jeju_travel</title>
...
<script type="text/javascript"> 
		function optionSelect2(f) {
				
				let option2 = document.getElementById("optionList").value
				let keyword = document.querySelector('.keyword').value;
				
				f.action = "search?option="+option2;
				f.action = "search?option="+option2+"&keyword=" + keyword; 
				f.submit();
			}
	</script>
</head>
				...
				<div class="wrapper_top_inner">
					<div class="wrapper_top_left">
						...
						<c:choose>
							...
							<c:otherwise>
								<select class="option_search" id="optionList" onchange="optionSelect2(this.form)" name="option_select2">
					...
					<td colspan="4">
						<ol class="paging">
							...
							<c:choose>
								...
								<c:otherwise>
									<li><a
										href="search?cPage=${paging3.beginBlock - paging3.pagePerBlock }&keyword=${keyword}&option=${option}">&#8249;</a></li>
								</c:otherwise>
							</c:choose>
							...
							<c:forEach begin="${paging3.beginBlock }"
								...
								<c:choose>
									...
									<c:otherwise>
										<li><a
											href="search?cPage=${k}&keyword=${keyword}&option=${option}">${k}</a></li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
