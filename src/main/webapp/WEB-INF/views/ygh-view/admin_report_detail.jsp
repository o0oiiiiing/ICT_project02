...
function report_ans_write_ok(f) {
    let reportOk = document.getElementById('reportOk').checked;
    let reportNo = document.getElementById('reportNo').checked;
    
    if (reportOk) {
    	f.action = "report_ans_write_ok.do?report=reportOk";
    	f.submit();
	} else if (reportNo) {
		f.action = "report_ans_write_ok.do?report=reportNo";
		f.submit();
	}
}
</script>
		...
		<table>
			...
			<tr>
				...
				<td style="text-align: left;">
					<label><input type="radio" id="reportOk" name="report">처리</label>
					<label><input type="radio" id="reportNo" name="report">미처리</label>
				</td>
			</tr>
			<tr>
				<th>내용</th>
